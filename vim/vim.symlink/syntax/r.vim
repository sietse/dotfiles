...
"                  --> NOT: instead, also clear all items. --Sietse
if version < 600
  syntax clear
...
" Comment
syn match rComment "#.*$" contains=rRoxygen,rTODO
...
syn match rArrow /[^=]\zs=\ze[^=]/

syn keyword rTodo     contained TODO FIXME XXX
syn match rRoxygen    contained "@param\|@require\|@author\|@aliases\|@concept\|@example\|@examples\|@keywords\|@method\|@name\|@note\|@param\|@references\|@return\|@seealso\|@title\|@TODO\|@usage" 


" Type
syn keyword rType function
" functions
syn match rFunction 	"\<\<[a-zA-Z\!\?_]\k*\>("me=e-1
" package names
...
" Error
syn region rRegion matchgroup=rDelimiter start=/(/ matchgroup=rDelimiter end=/)/ transparent contains=ALLBUT,rError,rBraceError,rCurlyError
syn region rRegion matchgroup=rDelimiter start=/{/ matchgroup=rDelimiter end=/}/ transparent contains=ALLBUT,rError,rBraceError,rParenError
syn region rRegion matchgroup=rDelimiter start=/\[/ matchgroup=rDelimiter end=/]/ transparent contains=ALLBUT,rError,rCurlyError,rParenError
syn match rError      /[)\]}]/
...
if version >= 508 || !exists("did_r_syn_inits")
  ...
  HiLink rFunction    Type
  HiLink rType		  Delimiter
  HiLink rPackages    Structure 
  HiLink rDelimiter   SpecialKey
  HiLink rError       Error
  HiLink rBraceError  Error
  HiLink rCurlyError  Error
  HiLink rParenError  Error
  HiLink rTodo        Todo
  HiLink rRoxygen     Normal
